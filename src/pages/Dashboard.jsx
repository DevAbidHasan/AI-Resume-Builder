        ...
        <div className="grid grid-cols-2 sm:flex flex-wrap gap-4">
          {allResumes.map((resume, index) => {
            ...
            return (
              ...
              >
                <FilePenLineIcon
                  className="size-7 group-hover:scale-120 transition-all"
                ...
                <p
                  className="absolute bottom-3 text-[11px] text-slate-400 group-hover:text-slate-500 transition-all duration-300 px-2 text-center"
                ...
                showCreateResume && (
                    <form onSubmit={createResume} onClick={()=> setShowCreateResume(false)} className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
                        <div onClick={e=>e.stopPropagation()} className="relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6">
                            ...
                            <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" name="" id="" placeholder="Enter resume title" className="w-full rounded-xl px-4 py-2 mb-4 focus:border-blue-600 focus:border" required />
                ...
                showUploadResume && (
                    <form onSubmit={uploadResume} onClick={()=> setShowUploadResume(false)} className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
                        <div onClick={e=>e.stopPropagation()} className="relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6">
                            ...
                            <button className="w-full py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">Upload Resume</button>
